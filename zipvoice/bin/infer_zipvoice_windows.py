...
def generate_sentence(
    ...
    total_voc_units = total_gen_units  # one vocoder chunk per generated chunk
    total_units = GEN_W * total_gen_units + VOC_W * total_voc_units
    done_units = 0
    # Start predicting features
    chunked_features = []
    start_t = dt.datetime.now()
    chunked_wavs_cpu = []  # list[(index, cpu_wav)]
    ...
    for batch_idx, batch_tokens in enumerate(tokens_batches):
        ...
        # Generate features
        print(f"Generating features for batch {batch_idx}...")
        (
            pred_features,
            pred_features_lens,
            pred_prompt_features,
        ...
        print(f"Postprocessing features for batch {batch_idx}...")
        # Postprocess predicted features
        #pred_features = pred_features.permute(0, 2, 1) / feat_scale  # (B, C, T)
        ...
        for i in range(pred_features.size(0)):
            ...
            wav_gpu = vocoder.decode(feat_i).squeeze(1).clamp(-1, 1)  # GPU
            if prompt_rms < target_rms:
                wav_gpu = wav_gpu * prompt_rms / target_rms
            wav_cpu = wav_gpu.cpu()  # <-- free GPU memory ASAP
            # Map back to the global chunk index
            global_chunk_index = chunked_index[sum(len(b) for b in tokens_batches[:batch_idx]) + i]
    ...
    print("Starting vocoder processing...")
    # Start vocoder processing
    start_vocoder_t = dt.datetime.now()
    ...
    wav_cpu = final_wav.detach().cpu()

    # 2. Convert to Numpy and fix shape
    # Torchaudio is [Channels, Time], Soundfile needs [Time] (for mono) or [Time, Channels]
    wav_numpy = wav_cpu.squeeze().numpy()

    # 3. Save using soundfile
