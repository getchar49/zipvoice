...
def load_prompt_wav(prompt_wav: str, sampling_rate: int):
    """
    Replacement for load_prompt_wav using Librosa.
    Does loading + resampling in one step.
    """
    # librosa.load automatically resamples to 'sr'. 
    # mono=False preserves the original channels (stereo), matching torchaudio's behavior.
    wav_numpy, _ = librosa.load(prompt_wav, sr=sampling_rate, mono=False)
    
    # Convert numpy array to PyTorch tensor
    wav_tensor = torch.from_numpy(wav_numpy).float()
    
    # Handle Shape differences:
    # Librosa Mono: (Time,) -> Needs to be (1, Time) for your model
    # Librosa Stereo: (Channels, Time) -> Already matches Torchaudio
